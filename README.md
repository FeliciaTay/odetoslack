...
Upon finishing the setting up process, this will be a Slack application which will be deployed on AWS Lambda through Zappa. Zappa is a Python package that bundles up the web application written in Flask. The backend database involved is MySQL database, which can be hosted on AWS RDS.
